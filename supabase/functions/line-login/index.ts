...
Deno.serve(async (request) => {
  try {
    ...
    const { data: exchange, error: exchangeError } = await supabase.from('line_login_exchange_codes').insert({ line_session_token: session.token, binding_hash: await sha256(browserBinding), expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString() }).select('code').single();
