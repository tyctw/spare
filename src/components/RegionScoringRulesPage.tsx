...
const REGION_RULES: Record<string, RegionRule> = {
  taipei: {
    ...
    overview: '基北區以志願序、多元學習表現及國中教育會考三個面向進行超額比序；會考積分同時保留標示與寫作測驗等細部比序資訊。',
    entryNote: '以下積分對照表適用於 111 學年度前入學之學生。',
    comparisonTable: [
      ...
      { category: '多元學習表現', maximum: '36 分', item: '均衡學習（上限 21 分）', conversion: ['7 分：符合 1 個領域', '0 分：未符合'], description: ['健體、藝術、綜合、科技四領域任三領域前五學期平均成績及格者。', '108 學年度前入學者，採計健康與體育、藝術與人文及綜合活動三領域；前五學期平均成績及格者，每一領域得 7 分，滿分 21 分。'] },
      { category: '多元學習表現', maximum: '36 分', item: '服務學習（上限 15 分）', conversion: ['5 分：每學期服務滿 6 小時以上'], description: ['由國民中學學校認證。', '非應屆畢業生或具同等學力資格者，由原畢業學校進行服務時數採計，並得採計畢業後服務時數至 114 學年度上學期止，比照在校生方式辦理。', '採計原則依「基北區免試入學服務學習時數認證及轉換採計原則」辦理。'] },
    ...
    rules: [
      ...
      { title: '多元學習表現', maximum: '36 分', description: '採計均衡學習、服務學習與日常表現等項目。', points: ['均衡學習最高 21 分。', '服務學習與日常表現的採計期間、認證方式及上限，依基北區公告辦理。'] },
  ...
  changhua: {
    ...
    ], exam: ['會考標示積分與寫作測驗均為超額比序資料，請依簡章的先後順序判讀。'], reminders: ['各子項上限不等於每位學生都能直接取得，需符合採計條件。'],
  },
  tainan: {
  ...
  kaohsiung: {
    ...
    rules: [
      ...
      { title: '國中教育會考', maximum: '30 分', description: '五科採精熟、基礎、待加強換算。', points: ['精熟 6 分、基礎 4 分、待加強 2 分。'] },
    ], exam: ['A++、A+、A 對應 6 分與 7、6、5 點；B++、B+、B 對應 4 分與 4、3、2 點；C 對應 2 分與 1 點。'], reminders: ['志願群與同校同群的認定會影響志願序積分，送出前須逐一核對。'],
  },
  hsinchu: {
...
export default function RegionScoringRulesPage({ regionId }: { regionId: string }) {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8"><div className="mb-8 rounded-2xl border-2 border-amber-400 bg-amber-50 p-5"><div className="flex gap-3"><AlertTriangle className="h-6 w-6 shrink-0 text-amber-700" /><div><h2 className="font-black">使用前請先確認招生管道</h2><p className="mt-1 text-sm font-bold leading-6 text-slate-700">本頁整理的是 115 學年度一般免試入學的超額比序架構。優先免試、完全免試、技優甄審及個別學校招生可能另有規定；正式報名與資格認定一律以官方簡章及原國中審查結果為準。</p>{data.entryNote && <p className="mt-3 inline-flex rounded-lg bg-amber-200 px-3 py-1.5 text-sm font-black text-amber-950">{data.entryNote}</p>}</div></div></div>
      {data.comparisonTable && <section className="mb-8"><div className="mb-4 flex items-center gap-2"><Calculator className="h-6 w-6 text-indigo-700" /><h2 className="text-2xl font-black">免試入學比序項目積分對照表</h2></div><div className="overflow-x-auto rounded-3xl border-4 border-slate-900 bg-white shadow-[5px_5px_0_0_#0f172a]"><table className="w-full min-w-[900px] border-collapse text-left"><thead className="bg-slate-900 text-white"><tr><th className="p-4 font-black">類別</th><th className="p-4 font-black">上限</th><th className="p-4 font-black">項目</th><th className="p-4 font-black">積分換算</th><th className="p-4 font-black">說明</th></tr></thead><tbody>{data.comparisonTable.map((row) => <tr key={`${row.category}-${row.item}`} className="border-t-2 border-slate-200 align-top"><td className="p-4 font-black">{row.category}</td><td className="p-4 font-black text-fuchsia-700">{row.maximum}</td><td className="p-4 font-black text-slate-800">{row.item}</td><td className="p-4"><ul className="space-y-1 text-sm font-bold leading-6 text-slate-700">{row.conversion.map((item) => <li key={item}>{item}</li>)}</ul></td><td className="p-4"><ul className="space-y-2 text-sm font-bold leading-6 text-slate-700">{row.description.map((item) => <li key={item}>{item}</li>)}</ul></td></tr>)}</tbody><tfoot><tr className="border-t-4 border-slate-900 bg-amber-200"><td colSpan={4} className="p-4 text-lg font-black">總積分</td><td className="p-4 text-lg font-black">{data.total} 分</td></tr></tfoot></table></div></section>}
