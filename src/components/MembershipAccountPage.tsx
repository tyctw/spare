        ...
        {state === 'loading' ? (
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr] lg:items-start">
            ...
            <section className="overflow-hidden rounded-[1.75rem] border-2 border-slate-900 bg-white shadow-[5px_5px_0_#161b35]">
              ...
              {purchases.length ? <div className="space-y-3 bg-slate-50/70 p-3">
                {(isHistoryExpanded ? purchases : purchases.slice(0, 1)).map((purchase) => {
                  ...
                  return <div key={`${purchase.reference}-${purchase.createdAt}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-3 border-t border-dashed border-slate-200 pt-3 text-xs font-bold leading-5 text-slate-500"><p>訂單末碼 <span className="break-all font-black text-slate-700">{purchase.reference}</span></p><p>{paid ? `付款日 ${formatDate(purchase.paidAt)} · 有效至 ${formatDate(purchase.expiresAt)}` : `建立日 ${formatDate(purchase.createdAt)}`}</p></div>
                  </div>;
                })}
                {purchases.length > 1 && (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-slate-300 py-3 text-xs font-black text-slate-600 transition hover:border-slate-400 hover:bg-slate-100"
                  >
