...
export default function Footer() {
  return <footer className="mt-16 border-t-4 border-slate-900 bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-7xl overflow-hidden rounded-3xl border-4 border-slate-900 bg-white shadow-[6px_6px_0_#0f172a]">
      ...
      <nav className="grid grid-cols-3 border-y-2 border-slate-900">{links.map(({ label, href, icon: Icon }) => <a key={href} href={withBasePath(href)} className="group flex items-center justify-center border-r-2 border-slate-900 px-2 py-3 text-center text-xs font-black text-slate-900 transition hover:bg-amber-100 last:border-r-0 sm:justify-between sm:px-5 sm:py-4 sm:text-sm"><span className="flex min-w-0 items-center gap-1.5 whitespace-nowrap sm:gap-2"><Icon className="h-4 w-4 shrink-0 text-indigo-700" />{label}</span><ArrowRight className="hidden h-4 w-4 transition group-hover:translate-x-1 sm:block" /></a>)}</nav>
