  ...
  return <footer className="mt-16 border-t-4 border-slate-900 bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-7xl overflow-hidden rounded-3xl border-4 border-slate-900 bg-white shadow-[6px_6px_0_#0f172a]">
      <div className="grid gap-7 p-6 sm:p-8 lg:grid-cols-[1.2fr_.8fr] lg:items-stretch">
        <section className="relative overflow-hidden rounded-2xl border-2 border-slate-900 bg-[#f5f7ff] p-5 shadow-[3px_3px_0_#0f172a] sm:p-6">
          <div aria-hidden="true" className="absolute -right-8 -top-8 h-28 w-28 rounded-full border-[18px] border-indigo-200/70" />
          <div className="relative flex items-start gap-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border-2 border-slate-900 bg-white text-indigo-700 shadow-[3px_3px_0_#0f172a]"><Compass className="h-7 w-7" /></div><div><p className="text-[10px] font-black tracking-[0.18em] text-indigo-700">ADMISSION COMPASS</p><h2 className="mt-1 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">TW 全國會考落點分析</h2></div></div>
          <p className="relative mt-5 max-w-xl border-l-4 border-rose-400 pl-4 text-sm font-bold leading-7 text-slate-700">把複雜的升學資訊整理得更清楚，陪你一步步找到適合自己的選擇。</p>
          <p className="relative mt-4 inline-flex items-center gap-2 rounded-full border-2 border-slate-900 bg-white px-3 py-1.5 text-xs font-black text-slate-600"><ShieldCheck className="h-3.5 w-3.5 text-indigo-700" />非政府官方機構・分析結果僅供參考</p>
        </section>
      ...
      <nav className="grid grid-cols-3 border-y-2 border-slate-900">{links.map(({ label, href, icon: Icon }) => { const isSupport = href === '/support'; return <a key={href} href={withBasePath(href)} className={`group flex items-center justify-center border-r-2 border-slate-900 px-2 py-3 text-center text-xs font-black transition last:border-r-0 sm:justify-between sm:px-5 sm:py-4 sm:text-sm ${isSupport ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'text-slate-900 hover:bg-amber-100'}`}><span className="flex min-w-0 items-center gap-1.5 whitespace-nowrap sm:gap-2"><Icon className={`h-4 w-4 shrink-0 ${isSupport ? 'text-white' : 'text-indigo-700'}`} />{label}</span><ArrowRight className="hidden h-4 w-4 transition group-hover:translate-x-1 sm:block" /></a>; })}</nav>
