      ...
      <div className="relative mx-auto w-full max-w-[110rem]">
        ...
        <section className="mt-9">
          ...
          {choices.length === 0 ? (
            ...
            <div className="mt-5 overflow-x-auto rounded-2xl border-2 border-slate-900">
              <table className="min-w-[40rem] w-full text-left text-sm">
                <thead className="bg-indigo-50 text-xs font-black text-slate-700">
                  <tr><th className="w-20 px-4 py-3 text-center">順序</th><th className="px-4 py-3">學校</th><th className="px-4 py-3">科別／班別</th><th className="px-4 py-3">類型</th><th className="px-4 py-3">地區</th></tr>
                </thead>
              ...
              {choices.map((choice: any, index: number) => (
                ...
                >
                  <td className="px-4 py-3 text-center"><span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-amber-300 font-black text-slate-950">{index + 1}</span></td>
                  <td className="whitespace-nowrap px-4 py-3 font-black text-slate-950">{choice.name}</td>
                  <td className="whitespace-nowrap px-4 py-3 font-bold text-sky-700">{choice.deptName}{choice.shift ? `（${choice.shift}）` : ""}</td>
                  <td className="px-4 py-3"><div className="flex flex-wrap gap-1.5">{choice.levelInfo && <a href={withBasePath("/school-types")} className="rounded-lg border border-amber-300 bg-amber-100 px-2 py-1 text-xs font-black text-amber-900">{choice.levelInfo}</a>}{choice.groupName && <a href={choice.groupName === "學術群" ? withBasePath("/general-comprehensive-high-school") : withBasePath("/vocational-encyclopedia") + "?group=" + encodeURIComponent(choice.groupName)} className="rounded-lg border border-sky-300 bg-sky-100 px-2 py-1 text-xs font-black text-sky-800">{choice.groupName}</a>}</div></td>
                  <td className="whitespace-nowrap px-4 py-3 font-bold text-slate-600">{choice.county || "--"}</td>
                </tr>
              ))}
                </tbody>
              </table>
