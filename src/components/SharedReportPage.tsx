            ...
            displayMode === 'table' ? <><p className="mt-5 mb-2 text-xs font-bold text-slate-500 sm:hidden">左右滑動查看完整欄位</p><div className="min-w-0 max-w-full overflow-x-auto rounded-2xl border-2 border-slate-900">
              ...
              {choices.map((choice: any, index: number) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-2 py-2.5 font-black text-indigo-700">{choice.score ?? choice.points ?? choice.minScore ?? "--"}</td><td className="px-2 py-2.5"><div className="flex flex-wrap gap-1.5">{choice.levelInfo && <a href={withBasePath("/school-types")} className="rounded-lg border border-amber-300 bg-amber-100 px-2 py-1 text-xs font-black text-amber-900">{choice.levelInfo}</a>}{choice.groupName && <a href={choice.groupName === "學術群" ? withBasePath("/general-comprehensive-high-school") : withBasePath("/vocational-encyclopedia") + "?group=" + encodeURIComponent(choice.groupName)} className="rounded-lg border border-sky-300 bg-sky-100 px-2 py-1 text-xs font-black text-sky-800">{choice.groupName}</a>}</div></td>
                  <td className="whitespace-nowrap px-2 py-2.5 font-bold text-slate-600">{choice.county || "--"}</td>
                </tr>
              ))}
                </tbody>
              </table>
            </div></> : <div className="mt-5 grid gap-4 sm:grid-cols-2">{choices.map((choice: any, index: number) => <article key={`${choice.code}-${choice.deptCode}-${index}`} className="rounded-2xl border-2 border-slate-900 bg-white p-4 shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]"><div className="flex items-start gap-3"><span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-300 font-black">{index + 1}</span><div className="min-w-0"><h3 className="font-black text-slate-950">{choice.name}</h3><p className="mt-1 font-bold text-sky-700">{choice.deptName}{choice.shift ? `（${choice.shift}）` : ''}</p><p className="mt-2 text-sm font-black text-indigo-700">志願序分數：{choice.score ?? choice.points ?? choice.minScore ?? "--"}</p><div className="mt-3 flex flex-wrap gap-1.5 text-xs font-black">{choice.levelInfo && <span className="rounded-lg bg-amber-100 px-2 py-1 text-amber-900">{choice.levelInfo}</span>}{choice.groupName && <span className="rounded-lg bg-sky-100 px-2 py-1 text-sky-800">{choice.groupName}</span>}{choice.county && <span className="rounded-lg bg-slate-100 px-2 py-1 text-slate-600">{choice.county}</span>}</div></div></div></article>)}</div>
          )}
        </section>
