    ...
    <main id="main-content" aria-labelledby="membership-page-title" className="min-h-screen overflow-hidden bg-[#f5f6ff] px-4 py-5 text-slate-900 sm:px-6 sm:py-10">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1.12fr_.88fr]">
          <section className="relative overflow-hidden rounded-[2.5rem] border-2 border-slate-900 bg-violet-100 p-5 text-slate-900 shadow-[4px_4px_0_#161b35] sm:p-6">
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPlanComparison(true)} className="mt-4 inline-flex items-center gap-2 rounded-xl border-2 border-violet-900 bg-white px-4 py-2.5 text-sm font-black text-violet-900 shadow-[2px_2px_0_#4c1d95] transition hover:-translate-y-0.5 hover:bg-violet-50"><Sparkles className="h-4 w-4 text-amber-500" />會員與非會員差在哪？<ArrowRight className="h-4 w-4" /></button>
      ...
      {showPlanComparison && <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setShowPlanComparison(false); }}>
        <section role="dialog" aria-modal="true" aria-labelledby="plan-comparison-title" className="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto rounded-[2rem] border-2 border-slate-900 bg-white p-5 shadow-[4px_4px_0_#161b35] sm:p-7">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black tracking-[.18em] text-violet-700">SMART UPGRADE</p><h2 id="plan-comparison-title" className="mt-1 text-2xl font-black sm:text-3xl">會員與非會員，差別在使用自由度</h2><p className="mt-2 text-sm font-bold leading-6 text-slate-600">先免費探索，想更專心規劃時再升級；一次付款，到期不自動續扣。</p></div><button type="button" aria-label="關閉會員比較" onClick={() => setShowPlanComparison(false)} className="rounded-xl border-2 border-slate-900 p-2 hover:bg-slate-100"><X className="h-5 w-5" /></button></div>
