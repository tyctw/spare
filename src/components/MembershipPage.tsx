    ...
    <main id="main-content" aria-labelledby="membership-page-title" className="min-h-screen overflow-hidden bg-[#f5f6ff] px-4 py-5 text-slate-900 sm:px-6 sm:py-10">
      ...
      {showPlanComparison && <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setShowPlanComparison(false); }}>
        <section role="dialog" aria-modal="true" aria-labelledby="plan-comparison-title" className="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto rounded-[2rem] border-2 border-slate-900 bg-white p-5 shadow-[4px_4px_0_#161b35] sm:p-7">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2"><div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-4 sm:p-5"><h3 className="text-lg font-black text-slate-700">非會員</h3><p className="mt-1 text-xs font-bold text-slate-500">適合先了解工具</p><ul className="mt-4 space-y-3 text-sm font-bold text-slate-600">{['使用公開升學資訊與基本探索功能','可建立一般分析與比較草稿','分享連結依一般期限管理','需依網站提示完成使用授權'].map(item => <li key={item} className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-slate-400" />{item}</li>)}</ul></div><div className="relative overflow-hidden rounded-2xl border-2 border-violet-900 bg-violet-600 p-4 text-white shadow-[3px_3px_0_#4c1d95] sm:p-5"><span className="absolute right-3 top-3 rounded-full bg-amber-300 px-2 py-1 text-[10px] font-black text-slate-900">推薦</span><h3 className="text-lg font-black">會員</h3><p className="mt-1 text-xs font-bold text-violet-100">適合持續規劃與家庭共編</p><ul className="mt-4 space-y-3 text-sm font-bold text-violet-50">{['全程免廣告，專心完成分析','無限次數落點分析與一分改變分析','家長協作志願表與版本比較還原','會員分享連結與期限管理','一次付款，到期不自動續扣'].map(item => <li key={item} className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-amber-300" />{item}</li>)}</ul></div></div>
