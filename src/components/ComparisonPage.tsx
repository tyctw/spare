import { ArrowLeft, ArrowUpDown, Download, ExternalLink, Filter, List, MapPin, Plus, Printer, RotateCcw, Search, SlidersHorizontal, Trash2, X } from 'lucide-react';
...
export default function ComparisonPage() {
  ...
  const filterOptions = React.useMemo(() => ({
    regions: Array.from(new Set(schools.map((school) => school.region).filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b), 'zh-Hant')),
  ...
  const filteredSchools = React.useMemo(() => schools
    ...
    .map(({ school }) => school), [schools, searchTerm, regionFilter, typeFilter, ownershipFilter, sortBy]);
  const hasFilters = Boolean(searchTerm || regionFilter !== 'all' || typeFilter !== 'all' || ownershipFilter !== 'all' || sortBy !== 'added');
  const clearFilters = () => {
    setSearchTerm('');
    setRegionFilter('all');
    setTypeFilter('all');
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {schools.length ? <>
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border-2 border-slate-900 bg-white p-4 shadow-[3px_3px_0_#0f172a] sm:flex-row sm:items-start sm:justify-between"><p className="pt-2 text-sm font-bold text-slate-600">可回到結果頁，將需要研究的校科持續加入清單。</p><div className="w-full sm:w-auto"><button type="button" onClick={() => setIsFieldSettingsOpen((open) => !open)} aria-expanded={isFieldSettingsOpen} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border-2 border-slate-900 bg-amber-100 px-4 py-2.5 text-sm font-black text-amber-800 sm:w-auto"><SlidersHorizontal className="h-4 w-4" />顯示欄位（{visibleFields.length}）</button>{isFieldSettingsOpen && <div className="mt-2 rounded-2xl border-2 border-slate-900 bg-slate-50 p-3 shadow-[3px_3px_0_#0f172a]"><div className="mb-3 flex items-center justify-between gap-3"><p className="text-xs font-black text-slate-600">至少保留一個欄位</p><button type="button" onClick={() => updateVisibleFields(defaultVisibleFields)} className="inline-flex items-center gap-1 text-xs font-black text-indigo-700 underline underline-offset-4"><RotateCcw className="h-3.5 w-3.5" />全部顯示</button></div><div className="grid grid-cols-2 gap-2 sm:min-w-[380px]">{comparisonFields.map((field) => <label key={field.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs font-black text-slate-700"><input type="checkbox" checked={visibleFields.includes(field.id)} onChange={() => toggleVisibleField(field.id)} className="h-4 w-4 accent-indigo-600" />{field.label}</label>)}</div></div>}<div className="mt-2 grid grid-cols-2 gap-2 sm:flex"><a href={withBasePath('/results')} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border-2 border-slate-900 bg-sky-100 px-3 py-2.5 text-sm font-black text-sky-800 sm:w-auto sm:px-4 sm:py-2"><Plus className="h-4 w-4" />新增學校</a><button onClick={clearSchools} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border-2 border-slate-900 bg-rose-100 px-3 py-2.5 text-sm font-black text-rose-700 sm:w-auto sm:px-4 sm:py-2"><Trash2 className="h-4 w-4" />清空</button></div></div></div>
        <section className="mt-6 rounded-2xl border-2 border-slate-900 bg-slate-100 p-4 shadow-[3px_3px_0_#0f172a]" aria-label="搜尋、篩選與排序比較清單"><div className="flex flex-col gap-3 lg:flex-row lg:items-end"><label className="block flex-1"><span className="mb-1.5 flex items-center gap-1.5 text-xs font-black text-slate-600"><Search className="h-3.5 w-3.5" />搜尋學校</span><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="校名、群別、類型或就學區" className="w-full rounded-xl border-2 border-slate-900 bg-white px-3 py-2.5 text-sm font-bold outline-none placeholder:text-slate-400 focus:border-indigo-600" /></label><label className="block"><span className="mb-1.5 flex items-center gap-1.5 text-xs font-black text-slate-600"><Filter className="h-3.5 w-3.5" />就學區</span><select value={regionFilter} onChange={(event) => setRegionFilter(event.target.value)} className="w-full rounded-xl border-2 border-slate-900 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-indigo-600 lg:w-32"><option value="all">全部</option>{filterOptions.regions.map((region) => <option key={region} value={region}>{region}</option>)}</select></label><label className="block"><span className="mb-1.5 text-xs font-black text-slate-600">類型</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="w-full rounded-xl border-2 border-slate-900 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-indigo-600 lg:w-32"><option value="all">全部</option>{filterOptions.types.map((type) => <option key={type} value={type}>{type}</option>)}</select></label><label className="block"><span className="mb-1.5 text-xs font-black text-slate-600">公私立</span><select value={ownershipFilter} onChange={(event) => setOwnershipFilter(event.target.value)} className="w-full rounded-xl border-2 border-slate-900 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-indigo-600 lg:w-28"><option value="all">全部</option>{filterOptions.ownerships.map((ownership) => <option key={ownership} value={ownership}>{ownership}</option>)}</select></label><label className="block"><span className="mb-1.5 flex items-center gap-1.5 text-xs font-black text-slate-600"><ArrowUpDown className="h-3.5 w-3.5" />排序</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value as typeof sortBy)} className="w-full rounded-xl border-2 border-slate-900 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-indigo-600 lg:w-36"><option value="added">加入順序</option><option value="name">校名</option><option value="region">就學區</option><option value="type">學校類型</option><option value="quota">招生名額（多→少）</option></select></label></div><div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs font-black text-slate-600">顯示 <span className="text-indigo-700">{filteredSchools.length}</span> ／ {schools.length} 所</p><div className="grid grid-cols-2 gap-2 sm:flex">{hasFilters && <button type="button" onClick={clearFilters} className="inline-flex items-center justify-center gap-1 rounded-lg border-2 border-slate-300 bg-white px-2 py-2 text-xs font-black text-slate-600 hover:text-slate-900"><X className="h-3.5 w-3.5" />清除條件</button>}<button type="button" onClick={exportComparison} disabled={!filteredSchools.length} className="inline-flex items-center justify-center gap-1 rounded-lg border-2 border-slate-900 bg-emerald-100 px-2 py-2 text-xs font-black text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40"><Download className="h-3.5 w-3.5" />匯出 Excel</button><button type="button" onClick={printCurrentComparison} disabled={!filteredSchools.length} className="inline-flex items-center justify-center gap-1 rounded-lg border-2 border-slate-900 bg-white px-2 py-2 text-xs font-black text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"><Printer className="h-3.5 w-3.5" />列印比較表</button></div></div></section>
        {filteredSchools.length ? <div className="mt-7 grid gap-6 md:grid-cols-2">{filteredSchools.map((school, index) => <SchoolComparisonCard key={school.name} school={school} index={index} visibleFields={visibleFields} onRemove={() => removeSchool(school.name)} />)}</div> : <section className="mt-7 rounded-3xl border-4 border-dashed border-slate-300 bg-white px-6 py-14 text-center"><Search className="mx-auto h-8 w-8 text-slate-400" /><h2 className="mt-3 text-xl font-black">找不到符合的學校</h2><p className="mt-2 text-sm font-bold text-slate-500">試試減少篩選條件或換一個搜尋關鍵字。</p><button type="button" onClick={clearFilters} className="mt-5 rounded-xl border-2 border-slate-900 bg-amber-100 px-4 py-2 text-sm font-black shadow-[2px_2px_0_#0f172a]">清除條件</button></section>}
