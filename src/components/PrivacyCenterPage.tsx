...
const button = 'rounded-xl border-2 border-slate-900 px-4 py-2 font-bold transition hover:-translate-y-0.5 disabled:opacity-40';
  ...
  return <main id="main-content" className="min-h-screen bg-[#f3f6f1] px-4 py-6 text-slate-900 sm:px-6 sm:py-10">
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      <section className={card} aria-labelledby="shares-title">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[11px] font-black tracking-[0.18em] text-emerald-700">SHARED ACCESS</p><h2 id="shares-title" className="mt-1 flex items-center gap-2 text-2xl font-black"><Link2 className="text-emerald-700" />我的分享連結</h2><p className="mt-2 text-sm font-bold text-slate-500">掌握誰能看見你的內容，以及連結何時失效。</p></div><button className={`${button} bg-slate-50`} disabled={busy} onClick={() => load()}>重新整理分享</button></div>
