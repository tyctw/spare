...
// 系統開放時間設定
const SYSTEM_OPEN_TIME = new Date('2025-08-08T12:00:00');

// 檢查系統是否開放
function checkSystemAvailability() {
  const currentTime = new Date();
  if (currentTime < SYSTEM_OPEN_TIME) {
    // 系統尚未開放
    const modal = document.createElement('div');
    modal.className = 'system-unavailable-modal';

    const content = document.createElement('div');
    content.className = 'system-unavailable-content';

    const title = document.createElement('h2');
    title.className = 'system-unavailable-title';
    title.textContent = '系統尚未開放';

    const message = document.createElement('p');
    message.className = 'system-unavailable-message';
    message.textContent = `系統將於 2025 年 8 月 1 日 中午 12:00 開放使用`;

    const countdown = document.createElement('p');
    countdown.className = 'system-unavailable-countdown';

    // 更新倒數計時
    function updateCountdown() {
      const now = new Date();
      const diff = SYSTEM_OPEN_TIME - now;
      ...
      countdown.innerHTML = `
        <div style="display: flex; justify-content: center; gap: 20px; margin-bottom: 10px;">
          <div>
            <span style="font-size: 2rem; font-weight: 800;">${formatNumber(days)}</span>
            <span style="font-size: 0.9rem; opacity: 0.8;">天</span>
          </div>
          <div>
            <span style="font-size: 2rem; font-weight: 800;">${formatNumber(hours)}</span>
            <span style="font-size: 0.9rem; opacity: 0.8;">時</span>
          </div>
          <div>
            <span style="font-size: 2rem; font-weight: 800;">${formatNumber(minutes)}</span>
            <span style="font-size: 0.9rem; opacity: 0.8;">分</span>
          </div>
          <div>
            <span style="font-size: 2rem; font-weight: 800;">${formatNumber(seconds)}</span>
            <span style="font-size: 0.9rem; opacity: 0.8;">秒</span>
          </div>
        </div>
        <div style="font-size: 1rem; opacity: 0.7;">距離系統開放</div>
      `;
    }
